...
const scrapeFilmweb = async (browser, vodService) => {
  ...
  const moviesOnService = await page.$$eval(".rankingType", (elements) => {
    return elements.slice(0, 10).map((element) => {
      ...
      const rating = ratingElement
        ? ratingElement.textContent.trim().replace(".", ",")
        : "";
  ...
  await page.close();
  return moviesOnService.map((movie) => ({
    ...movie,
    vodService: vodService.name,
  }));
};
...
const scrapeAndSaveData = async () => {
  ...
  const vodServices = [
    {
      name: "Netflix",
      url: `https://www.filmweb.pl/ranking/vod/netflix/film/${currentYear}`,
    },
    {
      name: "HBO Max",
      url: `https://www.filmweb.pl/ranking/vod/hbo_max/film/${currentYear}`,
    },
    {
      name: "Canal+",
      url: `https://www.filmweb.pl/ranking/vod/canal_plus/film/${currentYear}`,
    },
    {
      name: "Disney",
      url: `https://www.filmweb.pl/ranking/vod/disney/film/${currentYear}`,
    },
  ];

  const scrapePromises = vodServices.map((vodService) =>
    scrapeFilmweb(browser, vodService),
  );
...
scrapeAndSaveData()
  ...
  .catch((error) => {
    console.error(
      "An error occurred when trying to save data to the movies.csv file.",
      error,
    );
  });
